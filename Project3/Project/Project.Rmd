...
```{r More precise look at quality column.}
table(df$quality)
```

So it's more useful and suitable to create ordered factor.
```{r Ordered Factor,echo=FALSE}
...
ggplot(data=subset(df, fixed.acidity < quantile(fixed.acidity, 0.95) & volatile.acidity <
                     quantile(volatile.acidity, .95)), aes(y=fixed.acidity, x = volatile.acidity)) +
...
ggplot(data=subset(df, free.sulfur.dioxide < quantile(free.sulfur.dioxide, .95) & total.sulfur.dioxide <
                     quantile(total.sulfur.dioxide, .95)), aes(y=free.sulfur.dioxide, x = total.sulfur.dioxide)) +
...
ggplot(data=subset(df, density < quantile(density, .95) & pH < quantile(pH, .95)), aes(y=density, x = pH)) +
  geom_jitter(aes(color=quality.factor)) + scale_colour_brewer(type="seq", palette=3)+
...
```{r high absolute correlation among others variables}
...
```{r move to prediction, warning=FALSE, message=FALSE}
ggplot(data=subset(df,density < 1.005) , aes(x=alcohol, y=density, color=quality.factor)) + geom_point() +
...
```{r variables across different wine qualities, warning=FALSE, message=FALSE}
ggplot(data=subset(df, chlorides < 0.2), aes(x=pH, y = chlorides, color = quality.factor)) + geom_point() +
  facet_wrap(~quality.factor) + geom_smooth(colour='black')
```
...
```{r highest correlation variable by quality, warning=FALSE, message=FALSE}
ggplot(data=subset(df, density < 1.005), aes(x=density, y = residual.sugar, color = quality.factor)) + geom_point() +
  facet_wrap(~quality.factor) + geom_smooth(colour='black')
```

Investigate next relationship between alcohol and free.sulfur.dioxide by quality.

```{r relationship between alcohol and free.sulfur.dioxide by quality, warning=FALSE, message=FALSE}
...
```{r scatter plot of chlorides versus alcohol by quality, warning=FALSE, message=FALSE}
...
```{r naive bayes model as my main model, message=FALSE, warning=FALSE}
df_pred <- df
df_pred$quality <- NULL
ctrl <- trainControl(method = "repeatedcv",  repeats = 3)
fit <- train(quality.factor ~ ., data = df_pred, method = "nb", trControl = ctrl,legendplot = TRUE)
fit
```
...
```{r gaussian distributed parameters,warning=FALSE, message=FALSE}
plot(fit$finalModel)
```
...
```{r Final plots and Summary - 1, echo=FALSE}
...
```{r Final plots and Summary - 2 , warning=FALSE, message=FALSE,echo=FALSE}
ggplot(data=subset(df, density < quantile(density, .95) & pH < quantile(pH, .95)), aes(y=density, x = pH)) +
  geom_jitter(alpha=1/4, aes(color=quality.factor))+
...
#+scale_colour_brewer(type="seq", palette="Set1") +
  geom_abline(intercept = .992, slope = 0) + ylab("density") + xlab("pH") + ggtitle("Ph and density by quality Separation")
```
...
```{r Final plots and Summary - 3 , warning=FALSE, message=FALSE,echo=FALSE}
ggplot(aes(x=alcohol, y=chlorides, color = quality.factor), data = df) + geom_point(alpha = 0.01) +
